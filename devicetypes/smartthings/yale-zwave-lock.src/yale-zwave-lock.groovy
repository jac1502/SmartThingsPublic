 ...
 */
metadata {
	definition (name: "Yale Z-Wave Lock", namespace: "smartthings", author: "SmartThings", runLocally: false, minHubCoreVersion: '000.017.0012', executeCommandsLocally: true) {
		capability "Actuator"
		capability "Lock"
		capability "Polling"
		capability "Refresh"
...
def updated() {
	// Device-Watch pings if no device events received for 1 hour (checkInterval)
	sendEvent(name: "checkInterval", value: 1 * 30 * 30, displayed: false, data: [protocol: "zwave", hubHardwareId: device.hub.hardwareID, offlinePingable: "1"])
	...
	try {
		...
		if (!device.currentState("lock") || !device.currentState("battery") || !state.configured) {
			...
			hubAction = response(delayBetween(cmds, 10*1000))
		}
	} catch (e) {
...
def poll() {
	...
	if (!latest || !secondsPast(latest, 3 * 60) || secondsPast(state.lastPoll, 45 * 60)) {
		cmds << secure(zwave.doorLockV1.doorLockOperationGet())
		state.lastPoll = now()
	...
	if (state.assoc != zwaveHubNodeId && secondsPast(state.associationQuery, 11 * 60)) {
		cmds << zwave.associationV1.associationSet(groupingIdentifier:2, nodeId:zwaveHubNodeId).format()
		cmds << secure(zwave.associationV1.associationSet(groupingIdentifier:1, nodeId:zwaveHubNodeId))
		cmds << zwave.associationV1.associationGet(groupingIdentifier:2).format()
		cmds << "delay 3000"
		...
		} else if (!state.lastbatt || now() - state.lastbatt > 53*30*30*1000) {
			cmds << secure(zwave.batteryV1.batteryGet())
		}
	}
...
def updateCodes(codeSettings) {
	...
	if (set_cmds) {
		return response(delayBetween(set_cmds, 1200))
	}
	return null
}
